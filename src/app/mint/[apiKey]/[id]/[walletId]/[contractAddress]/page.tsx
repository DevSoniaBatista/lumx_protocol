...
import {NewMint, MintResponse, mintToken } from "../../../../../../services/lumxapi";
...
export default function Mint() {
    ...
    const initialContractId = params && Array.isArray(params.id) ? params.id[0] : params?.id || '';
    const initialContractIdVerified = Array.isArray(initialContractId) ? initialContractId[0] : initialContractId || '';
    const [contractId, setContractId] = useState<string>(initialContractIdVerified);

    const initialContractAddress = params && Array.isArray(params.contractAddress) ? params.contractAddress[0] : params?.contractAddress || '';
    const initialContractAddressVerified = Array.isArray(initialContractAddress) ? initialContractAddress[0] : initialContractAddress || '';
    const [contractAddress, setContractAddress] = useState<string>(initialContractAddressVerified);

    const [quantity, seQuantity] = useState<string>();
    const [uriNumber, setUriNumber] = useState<string>();

    const [message, setMessage] = useState<string>("");

    const [mint, setMint] = useState<NewMint>({
        apiKey: initialApiKeyVerified,
        contractId: initialContractIdVerified,
    ...
    useEffect(() => {
        if (apiKey && contractId && walletId && contractAddress) {
          setMint({ apiKey, contractId, walletId});
        }
      }, [apiKey, contractId, walletId, contractAddress]);
    

      function btnMintToken() {
        if (!mint) return;
        ...
        mintToken(mint)
            .then(mintResponse => {
                ...
                setMintResponse(mintResponse);
            })
            .catch(err => {
                const errorMessage = "Error: "+err.message || "An error occurred while minting Token.";
                  setMessage(errorMessage );
            });
    }
  
    function onInputChange(evt: React.ChangeEvent<HTMLInputElement>) {
      setMint(prevState => ({ ...prevState, [evt.target.id]: evt.target.value }));
    }
              ...
              <div className="-mx-4 flex flex-wrap gap-2 items-center mb-4">
                <div className="px-4 w-full md:flex-1">
                  <h2 className="capitalize font-bold text-2xl text-gray-900">
                    ...
                    <span>Mint Token from Lumx</span>
                  </h2>
                </div>
              </div>
    
              {/* Flex container para o formulário e a resposta */}
              <div className="flex flex-wrap md:flex-nowrap gap-8">
                {/* Coluna do Formulário */}
                <div className="w-full px-4 md:w-1/2">
                  <form>
                    {[
                      { id: "apiKey", label: "Api Key", value: mint?.apiKey },
                      { id: "contractId", label: "Contract Id", value: mint?.contractId },
                      { id: "walletId", label: "Wallet Id", value: mint?.walletId },
                      { id: "quantity", label: "Quantity", value: mint?.quantity },
                      { id: "uriNumber", label: "Uri Number - Token Number", value: mint?.uriNumber }
                    ].map(({ id, label, value }) => (
                      <div key={id} className="mb-4">
                        ...
                        <input
                          ...
                          className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-1/2 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                          required
                        />
                      </div>
                    ))}
                      ...
                      <table className="min-w-full max-w-2xl mx-auto divide-y divide-gray-200 table-fixed text-xs">
                        <thead className="bg-gray-50">
                          <tr>
                            ...
                            <th className="w-3/4 px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                              Value
                            </th>
                          </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                          {Object.entries(mintResponse).map(([key, value]) => (
                            <tr key={key}>
                              <td className="px-2 py-1 whitespace-normal break-words text-gray-900">
                                {key}
                              </td>
                          ...
                          <tr>
                              ...
                              <a href={`https://testnets.opensea.io/assets/amoy/${contractAddress}/${mintResponse.tokenId}`}  target="_blank" rel="noopener noreferrer" className="text-gray-400 underline">
                                    OpenSea 
                              </a>
