...
import {createProject, NewProject, ProjectResponse, BlockchainNameEnum} from "../../services/lumxapi";
...
export default function Project() {
    ...
    function btnCreateProject() {
        ...
        setMessage("Creating Project... wait...");
        ...
        createProject(project)
            ...
            .catch(err => {
                const errorMessage = "Error: "+err.message || "An error occurred while creating project.";
        ...
        <div className="container mx-auto px-4">
          <div className="-mx-4 flex flex-wrap gap-2 items-center mb-4">
            <div className="px-4 w-full md:flex-1">
              <h2 className="capitalize font-bold text-2xl text-gray-900">
                ...
                <span>Create a Project from Lumx</span>
              </h2>
            </div>
          </div>
          <div className="flex flex-wrap -mx-4">
            ...
            {projectResponse && (
                        ...
                        <td className="px-2 py-1 whitespace-normal break-words text-gray-500">
                          {value !== undefined && value !== null ? (
                            <>
                              {key === 'apiKey' && (
                                <>
                                  <Image src="/copy.png" onClick={() => handleCopyClick(projectResponse?.apiKey)}
                                    width="30" height="30" alt="Copy Api Key"/> {copied ? 'Copied!' : ''} 
                                </>
                              )}
                              {typeof value === 'string'
              ...
              <br/>
              {projectResponse && (
                <Link  href={`/wallet/${projectResponse?.apiKey}`} 
                  className="bg-black font-bold hover:bg-gray-800 inline-block px-8 py-2 rounded text-white">
                Create Wallet</Link>
              )}
            </div>
