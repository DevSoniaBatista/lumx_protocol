...
import {deployContract, ContractResponse, NewDeploy} from "../../../../../services/lumxapi";
...
export default function Deploy() {
  ...
  const [deploy, setDeploy] = useState<NewDeploy>();
  const [deployResponse, setDeployResponse] = useState<ContractResponse | null>(null);
  ...
  useEffect(() => {
    if (apiKey && contractId) {
      setDeploy({ apiKey, contractId });
    }
  }, [apiKey, contractId]);

  function btnDeployContract() {
      if (!deploy) return;

      setDeployResponse(null);
      setMessage("Deploying Contract... wait...");
      deployContract(deploy)
      .then(deployResponse => {
          setMessage("Contract deployed successfully!");
          setContractAddress(deployResponse.address);
          setDeployResponse(deployResponse);
      })
      .catch(err => {
          const errorMessage = "Error: "+err.message || "An error occurred while deploying the contract.";
  ...
  function onInputChange(evt: React.ChangeEvent<HTMLInputElement>) {
    setDeploy(prevState => ({ ...prevState, [evt.target.id]: evt.target.value }));
  }
      ...
      <div className="container mx-auto px-4">
        <div className="-mx-4 flex flex-wrap gap-2 items-center mb-4">
          <div className="px-4 w-full md:flex-1">
            <h2 className="capitalize font-bold text-2xl text-gray-900">
              ...
              <span>Deploy a Contract from Lumx</span>
            </h2>
          </div>
        </div>
        ...
        <form className="w-full max-w-md">
          ...
          <div className="w-full px-4 md:w-1/2">
            ...
            {deployResponse && (
              <div className="overflow-auto mt-4">
                <table className="min-w-full max-w-2xl mx-auto divide-y divide-gray-200 table-fixed text-xs">
                  ...
                  {Object.entries(deployResponse).map(([key, value]) => (
                    <tr key={key}>
            ...
            <br/>
            {deployResponse && (
              <Link href={`/mint/${apiKey}/${contractId}/${walletId}/${contractAddress}`} 
              className="bg-black font-bold hover:bg-gray-800 inline-block px-8 py-2 rounded text-white">
              Mint Token</Link>
            )} 
          </div>
