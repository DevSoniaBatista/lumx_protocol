...
import {createToken, TokenResponse, NewToken} from "../../../../../services/lumxapi";
...
export default function Deploy() {
  ...
  function btnCreateToken() {
    ...
    setMessage("Creating Token Type ...wait...");
    ...
    .catch(err => {
        const errorMessage = "Error: "+err.message || "An error occurred while creating the token.";
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (id === 'apiKey') {
      ...
      setToken((prevState) => ({ ...prevState, [id]: value }));
    }
  };
      ...
      <div className="container mx-auto px-4">
        <div className="-mx-4 flex flex-wrap gap-2 items-center mb-4">
          <div className="px-4 w-full md:flex-1">
            <h2 className="capitalize font-bold text-2xl text-gray-900">
              ...
              <span>Create Token Type from Lumx</span>
            </h2>
          </div>
        </div>
        ...
        <form className="w-full max-w-md">
                {[
                  ...
                  { id: "contractId", label: "Contract Id", value: contractId },
                  { id: "walletId", label: "Wallet Id", value: walletId },
                  { id: "name", label: "Name", value: token?.name  },
                  { id: "description", label: "Description", value: token?.description },
                  { id: "traits", label: "Traits", value: token?.traits},
                  { id: "maxSupply", label: "MaxSupply", value: token?.maxSupply},
                  { id: "imageUrl", label: "Image (Pinata URL)", value: token?.imageUrl },
  
                ].map(({ id, label, value }) => (
                  <div key={id} className="mb-4">
                    <label htmlFor={id} className="block mb-1 text-sm font-medium text-gray-900 dark:text-white">{label}:</label>
                    {id === 'traits'  || id === 'imageUrl'? (
                        <>
                          <input
                            type="text"
                            id={id}
                          ...
                          />
                          {id === 'traits'? (
                            <label htmlFor={id} className="block mt-2 text-sm font-medium text-gray-900 dark:text-white">
                              Example: {`{ rare: 'special', object: 'crown' }`}
                            </label>
                            ):  
                            <label htmlFor={id} className="block mt-2 text-sm font-medium text-gray-900 dark:text-white">
                            Required if non-fungible contract <br/> Example: {`https://ipfs.io/ipfs/QmajpM45Bd1qwUfR6YcocmSiiBytfnPeAJT6h4DJbRiHVD`}
                          </label>}
                        </>
                      ) : (
                        <input
          ...
          <div className="w-full px-4 md:w-1/2">
            ...
            <br/>
            {tokenResponse && (
              <Link href={`/deploy/${apiKey}/${contractId}/${walletId}`} 
              className="bg-black font-bold hover:bg-gray-800 inline-block px-8 py-2 rounded text-white">
